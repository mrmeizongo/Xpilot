...
static inline void handle_interrupts(timer16_Sequence_t timer, volatile uint16_t* TCNTn, volatile uint16_t* OCRnA)
{
    ...
    else
    {
        if (SERVO_INDEX(timer, Channel[timer]) < ServoCount && SERVO(timer, Channel[timer]).Pin.isActive == true)
            writePin(SERVO(timer, Channel[timer]).Pin, LOW); // pulse this channel low if activated
    }
...
ISR(TIMER1_COMPA_vect) { handle_interrupts(_timer1, &TCNT1, &OCR1A); }
#endif

#if defined(_useTimer3)
ISR(TIMER3_COMPA_vect) { handle_interrupts(_timer3, &TCNT3, &OCR3A); }
#endif
...
ISR(TIMER5_COMPA_vect) { handle_interrupts(_timer5, &TCNT5, &OCR5A); }
#endif
...
int Servo::readMicroseconds()
{
    ...
    if (this->servoIndex != INVALID_SERVO)
        pulsewidth = ticksToUs(servos[this->servoIndex].ticks); //   +TRIM_DURATION; // 12 aug 2009, removed 15 sept 2026
    else
        pulsewidth = 0;
