...
void Mode::init(void)
{
    ...
    rollPIDF = PIDF<int32_t, int16_t>{config().rPIDFConfig.Kp / Control::RESOLUTION, config().rPIDFConfig.Ki / Control::RESOLUTION,
                                      config().rPIDFConfig.Kd / Control::RESOLUTION, config().rPIDFConfig.Kf / Control::RESOLUTION,
                                      config().rPIDFConfig.iWindUpMax, config().filterConfig.processDT, config().filterConfig.lowPassFilterFreq};

    pitchPIDF = PIDF<int32_t, int16_t>{config().pPIDFConfig.Kp / Control::RESOLUTION, config().pPIDFConfig.Ki / Control::RESOLUTION,
                                       config().pPIDFConfig.Kd / Control::RESOLUTION, config().pPIDFConfig.Kf / Control::RESOLUTION,
                                       config().pPIDFConfig.iWindUpMax, config().filterConfig.processDT, config().filterConfig.lowPassFilterFreq};

    yawPIDF = PIDF<int32_t, int16_t>{config().yPIDFConfig.Kp / Control::RESOLUTION, config().yPIDFConfig.Ki / Control::RESOLUTION,
                                     config().yPIDFConfig.Kd / Control::RESOLUTION, config().yPIDFConfig.Kf / Control::RESOLUTION,
                                     config().yPIDFConfig.iWindUpMax, config().filterConfig.processDT, config().filterConfig.lowPassFilterFreq};
