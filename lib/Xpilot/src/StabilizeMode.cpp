...
void StabilizeMode::run(void)
{
    ...
    int32_t pitchDemand = stabilizeDemand(input_trpy[Radio::CHANNEL::PITCH],
                                          getIMU_RPY(IMU::Axis::Y_AXIS),
                                          config().flightConfig.maxPitchRateDegs,
                                          config().flightConfig.maxPitchAngleDegs,
                                          config().flightConfig.pitchAngleKp);

    output_trpy[Radio::CHANNEL::ROLL] = rollPIDF.Compute(rollDemand, getIMU_G(IMU::X_AXIS));
    output_trpy[Radio::CHANNEL::PITCH] = pitchPIDF.Compute(pitchDemand, getIMU_G(IMU::Y_AXIS));
    output_trpy[Radio::CHANNEL::YAW] = yawPIDF.Compute(input_trpy[Radio::CHANNEL::YAW], getIMU_G(IMU::Z_AXIS));
}